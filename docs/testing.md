...
源码基线 `ae1fe8b`。本次收敛范围只调整交付说明，不新增业务机制。

- Windows `scripts/check.ps1`：格式、单元测试、vet、build 全部通过；默认集成测试跳过。
- 随后独立 Docker MySQL/Redis 环境执行完整 `go test -race ./... -count=1` 通过，包含全部真实依赖集成测试及异步 HTTP 测试，集成包耗时 6.144s。
- 保留缓存回源对照和同步秒杀历史基线；没有新增异步性能提升结论。
- 当前范围见 [进度与冻结范围](progress.md)，日常启动见 [本地运行](quickstart.md)。
